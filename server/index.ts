...
import sellersApplyRouter from "../routes/sellers/apply.ts";
import sellersApproveRouter from "../routes/sellers/approve.ts";
import sellersRejectRouter from "../routes/sellers/reject.ts";
import adminSellersRouter from "../routes/admin/sellers.ts";
import sellersMeRouter from "../routes/sellers/me.ts";
...
app.use(cors({
  origin: isProd ? 'https://shopnish-9vlk.onrender.com' : true, // Allow all origins in development
...
(async () => {
  ...
  app.use("/api/sellers/me", sellersMeRouter);
  ...
  if (isProd) {
    ...
    app.get("*", (req, res) => {
      if (!req.path.startsWith("/api")) {
        const hostname = req.get('host')?.split(':')[0] || '0.0.0.0';
        res.send(`
          <!DOCTYPE html>
          <html>
            <head>
              <title>Redirecting...</title>
              <meta http-equiv="refresh" content="0; url=http://${hostname}:5173${req.path}">
            </head>
            <body>
              <p>Redirecting to development server...</p>
              <script>
                window.location.href = 'http://${hostname}:5173${req.path}';
              </script>
            </body>
          </html>
        `);
